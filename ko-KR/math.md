...
**[이 챕터에서 사용되는 모든 코드는 여기서 찾을 수 있다.](https://github.com/quii/learn-go-with-tests/tree/main/math)**

현대 컴퓨터의 모든 파워는 큰 계산을 빛의 속도로 수행하기 때문에, 평균적인 개발자는 일하면서 거의 수학을 사용하지 않는다.
그러나 오늘은 아니다! 오늘은 _진짜_ 문제를 해결하기 위해 수학을 사용할 예정이다.
그리고 지겨운 수학이 아니다. - 우리는 삼각법(trigonometry), 벡터를 비롯해 고등학교 졸업 이후 사용할 일이 없었던 수학들을 사용할 것이다.

## 문제

우리는 시계의 SVG를 만들고 싶다. 디지털 시계가 아니다. -그랬다면 쉬웠을거다.- 침이 있는 _아날로그_ 시계이다.
화려한게 아니라 단순히 `time`패키지로부터 `Time`을 가져와서 모든 초침 -시침, 분침, 초침- 이 올바른 방향을 향하는 시계 SVG를 만드는 좋은 함수를 원한다.
그게 얼마나 어려울까?
